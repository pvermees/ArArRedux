...
# returns the indices of timevec2 that are closest to timevec1
...
getindices.redux <- function(X,prefix=NULL,num=NULL,den=NULL,
    ...
    if (is.null(pos)) {
        ...
        i4 <- getindices.logratios(X,j)
    }
    if (is.null(hop)) {
        i5 <- i
    } else {
        matches <- X$hop %in% hop
        if (invert){
            j <- which(!matches)
        } else {
            j <- which(matches)
        }
        i5 <- getindices.logratios(X,j)
    }
    return(which((i %in% i1) & (i %in% i2) &
                 (i %in% i3) & (i %in% i4) & (i %in% i5)))
}
...
concat.default <- function(lrlist,...){
    if (length(lrlist)==2) {
        ...
        y <- lrlist[[2]]
        out <- x
        if ('hop' %in% names(x) && 'hop' %in% names(y))
            out$hop <- c(x$hop,y$hop)
...
concat.WiscAr <- function(lrlist){
    ...
    out$hop <- NULL
    for (hop in names(lrlist)){
        out$hop <- append(out$hop,
                          rep(hop,nruns(lrlist[[hop]])))
    }
